// ============================================================
// Jack's Crackers — Dashboard Backend v4
// Google Apps Script Web App
// ============================================================
// SETUP:
// 1. Open your Google Sheet → Extensions → Apps Script
// 2. Create/replace the "Dashboard" script file with this code
// 3. Run installTriggers() ONCE manually (sets up 5pm digest)
// 4. Deploy → New deployment → Web app
//    - Execute as: Me
//    - Who has access: Anyone with Google Account
// 5. Copy the deployment URL into src/App.jsx → APPS_SCRIPT_URL
// ============================================================

// ── CONFIG ────────────────────────────────────────────────────
const SPREADSHEET_ID   = SpreadsheetApp.getActiveSpreadsheet().getId();
const BAKE_PLANNER_TAB = "Bake Planner";    // exact tab name — verify yours
const BAKE_LOG_TAB     = "Bake Log";        // created automatically
const BAGGER_LOG_TAB   = "Bagger Log";      // created automatically
const ALERTS_TAB       = "Dashboard Alerts"; // created automatically
const ORDERS_TAB       = "Orders";

const ON_HAND_COLUMN   = 3;   // A=1, B=2, C=3 — column holding On Hand

const MANAGER_EMAIL    = "kevin@jackscrackers.com";
const DIGEST_HOUR      = 17;  // 5pm — uses your Google account timezone

// ── PIN CONFIG ────────────────────────────────────────────────
// Set a unique 4-digit PIN for each role.
// Change these before deploying — do not leave as defaults.
const PINS = {
  admin:  "0000",   // Kevin — change this
  baker:  "0000",   // Kitchen assistant — change this
  bagger: "0000",   // Bagger — change this
};
...
const FLAVOR_ROW_MAP = {
  ...
  "Buttermilk Bacon":      6,
  "Lavender Rosemary":     7,
  "Cracked Pepper & Sage": 8,
  "Spicy Chocolate Mint":  9,
  "Chocolate Graham":      10,
  "Graham Crackers":       11,
};
...
// ── verifyPin ─────────────────────────────────────────────────
// Checks the submitted PIN against the stored value for that role.
// Returns { success, valid } — never returns the stored PIN.
// Rate limiting is handled client-side (5 attempts, 5-min lockout).
//
function verifyPin(data) {
  const role = data.role;
  const pin  = String(data.pin || "").trim();
  if (!PINS[role]) return { success: false, error: "Unknown role" };
  const valid = pin === String(PINS[role]);
  return { success: true, valid };
}

// ── logBake ───────────────────────────────────────────────────
// Receives multi-flavor, multi-size payload from baker form.
// Writes retail-equivalent bag counts to On Hand in Bake Planner.
// Sends immediate email + posts dashboard alert.
//
...
// ── logBagging ────────────────────────────────────────────────
// Bag count per flavor from bagger form.
// Logs to Bagger Log. Posts dashboard alert.
// No immediate email — included in 5pm digest instead.
//
function logBagging(data) {
  ...
  // Dashboard alert only — digest handles the email
...
// ── getAlerts ─────────────────────────────────────────────────
// Returns all unread alerts for the dashboard notification panel.
//
function getAlerts() {
  const ss    = SpreadsheetApp.openById(SPREADSHEET_ID);
...
// ── clearAlert ────────────────────────────────────────────────
// Marks an alert as read when Kevin dismisses it.
//
...
// ── sendDailyDigest ───────────────────────────────────────────
// Triggered at 5pm. Aggregates all bagging sessions from today
// and sends a summary email. Skips if nothing was logged today.
//
...
function buildBakeSummaryLine(flavors) {
  ...
  for (const [flavor, fd] of Object.entries(flavors)) {
    ...
    for (const [skey, cfg] of Object.entries(SIZE_CONFIG)) {
      ...
      if (b > 0) bagParts.push(b + " × " + cfg.label);
    }
...
function buildBakeEmailHtml(timestamp, flavors, updated) {
  ...
  for (const [flavor, fd] of Object.entries(flavors)) {
    ...
    for (const [skey, cfg] of Object.entries(SIZE_CONFIG)) {
      ...
      h += "<strong>" + bags + "</strong> × " + cfg.label;
...
// In the Apps Script editor: select installTriggers from the
// function dropdown and click Run.
//
function installTriggers() {
  // Remove any existing digest triggers to prevent duplicates
